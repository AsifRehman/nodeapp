...
$(document).ready(function(){
		$.fn.quickSubmitDrCr = function()
		{
			...
			if($("td.creditColumn").length){
				$("td.creditColumn").each(function(index, element){
					thisPageAmount += parseFloat($(this).text())||0;
				});
			}
			thisPageAmount = parseFloat(thisPageAmount)||0;
			amount = parseFloat(amount)||0;
			var alreadyCredited = amount+thisPageAmount;
			var cashInHand = $("input.cashball").val();
			var thenWhat = cashInHand - alreadyCredited;
			if(thenWhat<0){
				//displayMessage('Voucher Transaction Amounts Exceeds Cash Balance.');
				//return false;
			}
			if(amount!="" && narration != "" && accCode != "" && accTitle != ""){
				$(this).blur();
				// $("<tr class=\"transactions amountRow\"><td style=\"text-align:right;\" class='accCode'>"+accCode+"</td><td style=\"text-align:left;\" class='accTitle'>"+accTitle+"</td><td style=\"text-align:left;\" class='narration'>"+narration+"</td><td style=\"text-align:center;\" class=\"creditColumn\">"+amount+"</td><td style=\"text-align:center;\"><a class=\"pointer\" title=\"Delete\">X</a></td></tr>").insertAfter($(".transactions").first());
				...
				TString += "<td style=\"text-align:center;\"><a class=\"pointer\" title=\"Delete\">X</a></td>";
				TString += "</tr>";
		...
		$.fn.saveJournal = function(){
			...
			$.post('db/insert-cp.php',{newJv:true,
									invoiceNo : invoiceNo,
									jv_id : jv_id,
									...
									function(data){
										...
										if(data['OK'] == 'Y'){
											...
											$(".saveJournal").text("Update");
											var $print_button = '<a class="button pull-right mr-10" target="_blank" href="voucher-cash-payment.php?id='+data['ID']+'"><i class="fa fa-print"></i> Print </a>';
...
var getBillsListOfAccounts = function(account_code){
	...
	$.post('db/cash-payment.db.php',{mode:mode,supplierAccCode:account_code},function(data){
		data = $.parseJSON(data);
		...
		if(data['DATA'] == 'YES')
		{
			...
			bills.forEach(function(item, index)
			{
				remainingAmt = item['TOTAL_AMOUNT']-item['ALERT_BILL_AMOUNT'];
  				title = "Bill:"+item['BILL_NO']+", "+item['SUPP_ACC_CODE']+", Amt:"+remainingAmt;
...
var getCashBalance = function(){
	...
	$.post('db/get-account-balance.php',{supplierAccCode:cash_in_hand},function(data){
		data = $.parseJSON(data);
		$(".cashBalance").val(data['BALANCE']);
		$("input.cashball").val(data['AMOUNT']);
	});
};
